...
#        print(book_rating['title'])
        print('book id: [{}] rating: [{}]'.format(book_id, book_rating['title']))
        return api_key.group(1), book_id, book_rating['title']


def parse_concrete_book(url):
    api_key, book_id, book_rating = parse_book_id(url)
    url = 'https://www.pricelib.ru/api/bookprice?callback=plcallback&api_key={}&edition_id={}'.format(api_key, book_id)
    resp = requests.get(url, headers=headers)
    if resp.status_code == 404:
        print('requested page [{}] not found'.format(url))
    if resp.status_code == 200:
...
#        print(cleanup)
        ...
        a = list(filter(lambda x: x['is_available'] == "1" and x['type'] == 'paper', book_prices['data']))
#        print(json.dumps(a))
        for json_dict in a:
            print('{} {}'.format(json_dict['shop_title'], json_dict['price_discount']))
...
def ll_request(book):
    try:
        search_headers = {'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.100 Safari/537.36', 'Origin': 'https://www.livelib.ru',
